...
float3 fresnel(float3 wPos, float3 norm, int lightIdx, float metalness, float3 albedo)
{
	float3 v = vecToCam(wPos);
	float3 l = vecToLight(lightPos[lightIdx].xyz, wPos);
	float3 h = normalize(l + v);

	float3 F0 = float3(0.04f, 0.04f, 0.04f) * (1 - metalness) +  albedo * metalness;
	return F0 + (1 - F0) * pow(1 - posDot(h, v), 5);
}

float3 FresnelSchlickRoughnessFunction(float3 wPos, float3 norm, float roughness, float metalness, float3 albedo)
{
...
//float3 CountPBRColor(float3 wPos, float3 n, float3 v, float roughness, float metalness, float3 dielectricF0, float3 F0) {
float3 CountPBRColor(float3 wPos, float3 n, float3 v, float roughness, float metalness, float3 albedo) {
	...
	for (uint i = 0; i < lightCount.x; ++i)
	{
		...
		float3 F = fresnel(wPos, n, i, metalness, albedo);
	...
	float3 F = FresnelSchlickRoughnessFunction(wPos, n, roughness, metalness, albedo);
	float3 kS = F;
