...
HRESULT IRRGenerator::CompileShaderFromFile(const WCHAR* szFileName, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3DBlob** ppBlobOut)
{
  HRESULT hr = S_OK;
...
HRESULT IRRGenerator::Init(ID3D11Device* device, ID3D11DeviceContext* context) {
  ...
  hr = D3DCompileFromFile(L"CMToIRRMGenerator_PS.hlsl", NULL, NULL, "main", "ps_5_0", flags, 0, &pixelShaderBuffer, NULL);
  if (FAILED(hr))
    return hr;
